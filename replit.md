...
- The **prop line itself** — picked by `chooseBalancedLine(values)`, which searches candidate lines at `avg10 ± 1.0` in 0.5 steps and selects the one whose historical hit rate is closest to 50% (the same "split the action" approach real sportsbooks use). Push-aware (`v === line` counts half) so integer lines stay fair. Side selection (Over vs Under) is unified across the model and all factor builders via `chooseSide()` — the side with the higher historical hit rate wins, with recent form as tiebreaker. **All** stat categories with non-zero production are surfaced on the player detail sheet (~8 stat lines per player avg); the Board's `bestPick` headline tier-prefers props that are both balanced (hitRate10 in [0.3, 0.7]) AND have meaningful volume (line >= 1). Some low-volume MLB stats only support a 0.5-floor line, where no balanced alternative is mathematically possible — these still surface as the player's best available pick. Explicitly labeled "model line" in every prop's `reasoning`. PrizePicks and Underdog do NOT publish public APIs, and we do not bypass their authenticated mobile endpoints. To pull real sportsbook lines instead, set the optional `ODDS_API_KEY` env var.
